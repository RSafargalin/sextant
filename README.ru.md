...
`map` и `api` покрывают те же четыре языка. `map` и части `api` по Objective-C и C читают
объявления из индекса, а не вторым парсером, — для этого нужен собранный индекс. Заголовки C++
идут через clang: индекс не несёт уровней доступа, а поверхность, которая не отличает приватный
член от публичного, публичной поверхностью не является.

Структурные команды — `search` и `lint` — разбирают текст исходника, которого в индексе нет.
Swift идёт своим парсером, Objective-C, C и C++ — через clang, на тех самых флагах, с которыми
собирался каждый файл ([ADR-0004](docs/adr/0004-structural-layer-for-c-family.md)). Флаги даёт
`sextant index`, поэтому без собранного индекса такие файлы не читаются — и называются, а не
пропускаются молча. `changed` читает те же четыре языка: текст прошлой ревизии разбирается на
флагах, с которыми файл собирается сегодня. `construct` — эвристика и знает форму создания
объекта в каждом языке.
...
| `search <pattern>` | структурный поиск по AST (`$X`, вариадик `$$$`, statement-паттерны); Swift своим парсером, Objective-C/C/C++ через clang | синтаксис |
| `lint` | структурные правила гигиены (`--rules <json>` заменяет встроенный набор; `{"extends": "builtin"}` дополняет его) по тем же языкам, что и `search` | синтаксис |
...
| `changed` | символьный git-дифф: что добавлено, удалено, сменило сигнатуру; несравнённое называется | синтаксис |
| `golden` / `bench` | регрессии семантики по спеке / латентность и объём вывода | измеримость |
| `adoption` | сколько навигации по коду прошло через sextant, а не через grep, и что прошло мимо | измеримость |
| `hook` | записывает одно событие вызова инструмента для хука клиента (`--install` объясняет) | измеримость |
...
| `index` | собрать index store и снять флаги компиляции C-семейства: SPM (`swift build`) или app-таргет (`--app`, xcodebuild) | сборка |
...
полностью: под git напрямую, а вне его — передавая каталог самому git как рабочее дерево с
git-каталогом в кэше, так что внутри проекта ничего не создаётся. Чтобы исключить файлы, которые
отслеживаются намеренно — сгенерированный код, вендоренные исходники, снапшот-фикстуры, —
перечислите глобы в `exclude` в `.sextant.json` или передайте `--exclude` (повторяемый, и он
заменяет список из конфига, а не дополняет его):
...
Примеры по задачам — вопрос, команда и её настоящий вывод — в
[docs/recipes.ru.md](docs/recipes.ru.md).

`sextant init --client <имя>` регистрирует сервер для другого клиента — `claude-code` (по
умолчанию, файл в проекте) или `claude-desktop` (один список на машину). Предлагаются только
клиенты, проверенные на запущенном приложении: конфиг, написанный по документации и ни разу не
запущенный, отказывает молча, а виноватым выглядит инструмент.

## Автодополнение в оболочке

```bash
sextant completion zsh > "${fpath[1]}/_sextant"     # затем перезапустить оболочку
sextant completion bash > /usr/local/etc/bash_completion.d/sextant
```

Скрипт генерируется из каталога команд, поэтому предлагает ровно те команды и флаги, которые есть
у установленного бинаря, — после обновления его стоит перегенерировать.
...
### Что выполняет `index` и что его сдерживает

Все остальные команды читают. `index` — единственная, которая запускает код самого проекта:
`Package.swift`, плагины сборки и макросы это код, а с `--app` фаза `Run Script` — это shell-скрипт.
Запускайте на коде, который собрали бы руками. Если проект уже собран, `sextant index --no-build`
найдёт готовый стор и не выполнит ничего.

Два пути защищены по-разному — замер на macOS 26.5.2 / Swift 6.2.3 враждебным манифестом
и враждебным плагином сборки:

| | `index` (SwiftPM) | `index --app` (xcodebuild) |
|---|---|---|
| Запись в домашний каталог | запрещена SwiftPM | **разрешена** |
| Сеть из манифеста или плагина | запрещена SwiftPM | **разрешена** |
| Чтение домашнего каталога | **разрешено** | **разрешено** |

SwiftPM держит манифест и плагины в песочнице: чужой код может прочитать домашний каталог, но не
может ни записать в него, ни отправить прочитанное — для утечки нужна вторая стадия. Сама
компиляция, включая макросы, идёт через `swiftc`, а не SwiftPM, и в этот замер не попала. У пути
`--app` песочницы нет вообще: фаза `Run Script` выполняется с полными правами.

Обернуть сборку в `sandbox-exec` невозможно: SwiftPM применяет собственную песочницу, а вложенную
ядро запрещает — сборка падает даже под полностью разрешающим внешним профилем.
