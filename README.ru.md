...
пропускаются молча. `changed` по-прежнему только по Swift и перечисляет несравнённые файлы C,
C++ и Objective-C; `construct` — эвристика и знает форму создания объекта в каждом языке.
